...
//					Core.bitwise_and(webcam_image, webcam_image, maskedFrame ,fgMask); 
					 ...
					 Core.flip(webcam_image,webcam_image,1);
					   ...
					   Mat roi=webcam_image.colRange(new Range(50,350)).rowRange(50, 250);
					   Imgproc.rectangle(webcam_image, new Point(50,50), new Point(350,250), new Scalar(0, 255, 0),3);
...
//				        // intensities to be considered 'skin'
				        Scalar lower_skin = new Scalar(0,10,60);
...
//				        Imgproc.GaussianBlur(mask, mask, ksize, 100);
				        ...
				        Mat hierarchy = new Mat(hsvFrame.rows(),hsvFrame.cols(),CvType.CV_8UC1,new Scalar(0));
						Imgproc.findContours(hsvFrame ,contours, hierarchy, Imgproc.RETR_TREE, Imgproc.CHAIN_APPROX_SIMPLE);
						//Imgproc.drawContours(hsvFrame, contours, -1, new Scalar(255, 0, 0),-1);
					       double maxArea = 0;
					...
					panel5.setimagewithMat(hsvFrame);
					frame1.repaint();
					frame3.repaint();
				} else {
