		...
		if (capture.isOpened()) {
			while (true) {
				...
				if (!webcam_image.empty()) {
					...
					Imgproc.cvtColor(roi, hsvFrame, Imgproc.COLOR_BGR2HSV, 3);
//				        // define the upper and lower boundaries of the HSV pixel
//				        // intensities to be considered 'skin'
					Scalar lower_skin = new Scalar(0, 10, 60);
					Scalar upper_skin = new Scalar(20, 150, 255);
					...
					Imgproc.dilate(hsvFrame, mask, kernel, anchor, iterations);
					...
					try {
						epsilon = 0.0005 * Imgproc.arcLength(new MatOfPoint2f(max_contour.toArray()), true);
